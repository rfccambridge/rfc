...
namespace Robocup.MotionControl
{
    ...
    public class TangentBugPlanner : IPathPlanner
    {
        ...
        public TangentBugPlanner()
        {
            ...
            previousAngles = new double[NUM_ROBOTS];
        ...
        public RobotPath GetPath(Team team, int id, RobotInfo desiredState, IPredictor predictor, double avoidBallRadius)
        {
            ...
            double bestAngle;
            // find closest angle to center in both directions
            double bestAngleCCW, bestAngleCW;
            angle_range.ClosestToCenter(-Math.PI, Math.PI, out bestAngleCW, out bestAngleCCW);

            // both directions are bad - no path
            if ((bestAngleCW == -Math.PI) && (bestAngleCCW == Math.PI))
            {
                Console.WriteLine("NO GOOD ROUTE ({0})!!!", id);
                previousAngles[id] = 0;
                return new RobotPath(team, id);
            }

            if ((bestAngleCW > -Math.PI) && (bestAngleCCW == Math.PI))
                bestAngle = bestAngleCW;
            else if ((bestAngleCW == -Math.PI) && (bestAngleCCW < Math.PI))
                bestAngle = bestAngleCCW;
            else //hysteresis: chose the side last chosen for this robot
            {
                //bestAngle = Math.Min(bestAngleCCW, Math.PI + bestAngleCW);
                Console.WriteLine("CCW: {0}, CW: {1}", bestAngleCCW, Math.PI + bestAngleCW);
                //bestAngle = (previousAngles[id] > 0) ? bestAngleCCW : bestAngleCW;
                Vector2 wptCW = start + finalDirection.rotate(bestAngleCW);
                Vector2 wptCCW = start + finalDirection.rotate(bestAngleCCW);
                double deltaDist = Math.Sqrt(wptCW.distanceSq(desiredState.Position)) -
                    Math.Sqrt(wptCCW.distanceSq(desiredState.Position));
                double deltaAngle = UsefulFunctions.angleDifference(currentState.Velocity.cartesianAngle(),
                        directionToGoal.cartesianAngle());

                if(Math.Abs(deltaAngle) < Math.PI / 60)
                //if (Math.Abs(deltaDist) < 0.05) //Difference too small - hysteresis
                    //bestAngle = (previousAngles[id] > 0) ? bestAngleCCW : bestAngleCW;
                    bestAngle = UsefulFunctions.angleDifference(currentState.Velocity.cartesianAngle(),
                        directionToGoal.cartesianAngle()) > 0 ? bestAngleCCW : bestAngleCW;
                else
                    //bestAngle = (deltaDist > 0) ? bestAngleCCW : bestAngleCW;
                    bestAngle = (bestAngleCCW < Math.PI + bestAngleCW) ? bestAngleCCW : bestAngleCW;
            }

            // Once a side has been picked, stick to it
            // THIS IS A LITTLE BIT OF A HACK!
            //if (previousAngles[id] > MIN_ABS_VAL_STICK)
            //{
            //    bestAngle = angle_range.ClosestToCenter(-MIN_ABS_VAL_STICK, 100);
            //}
            //else if (previousAngles[id] < -MIN_ABS_VAL_STICK)
            //{
        ...
        public void ReloadConstants()
        {
            ...
            NUM_ROBOTS = Constants.get<int>("default", "NUM_ROBOTS");
