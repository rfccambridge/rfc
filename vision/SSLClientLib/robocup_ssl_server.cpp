...
bool RoboCupSSLServer::open() {
  ...
  Net::Address multiaddr,iface; // renamed because "interface" is reserved in Win32
  multiaddr.setHost(_net_address.c_str(),_port);
  if(_net_interface.length() > 0){
    iface.setHost(_net_interface.c_str(),_port);
  }else{
    iface.setAny();
  }

  if(!mc->addMulticast(multiaddr,iface)) {
...
bool RoboCupSSLServer::send(const SSL_WrapperPacket & packet) {
  ...
  // Replaced the mutex calls with the Win32 versions
