	...
	public class FloodFiller : AbstractFloodFiller {
		...
		public override void FloodFill(Bitmap bmp, Point pt)
		{
			...
			//m_fillcolor=BGRA(GetB(m_fillcolor),GetG(m_fillcolor),GetR(m_fillcolor),GetA(m_fillcolor));
              m_fillcolor = BGRA(GetB(m_fillcolor), GetG(m_fillcolor), GetR(m_fillcolor), 255);
		...
		public unsafe void QueueFloodFill(byte* scan0, int x, int y,Size bmpsize, int stride, byte* fillcolor)
		{
			...
			{
				//start the loop
				QueueFloodFill4(scan0,x,y,bmpsize,stride,fillcolor);
				//call next item on queue
				while(CheckQueue.Count>0)
				{
					Point pt=(Point)CheckQueue.Dequeue();
					QueueFloodFill4(scan0,pt.X,pt.Y,bmpsize,stride,fillcolor);
				}
			}else{
				//start the loop
				QueueFloodFill8(scan0,x,y,bmpsize,stride,fillcolor);
				//call next item on queue
				while(CheckQueue.Count>0)
				{
					Point pt=(Point)CheckQueue.Dequeue();
					QueueFloodFill8(scan0,pt.X,pt.Y,bmpsize,stride,fillcolor);
				}
			}
		}
		
    	//public unsafe void QueueFloodFill8(byte* scan0, int x, int y,Size bmpsize, int stride, byte* startcolor)
        public unsafe void QueueFloodFill8(byte* scan0, int x, int y, Size bmpsize, int stride, byte* fillcolor)

    	{
    		...
    		int* p=(int*) (scan0+(CoordsToIndex(x,y, stride)));
        	//if (!(PixelsChecked[x,y]) && CheckPixel((byte*) p,startcolor)) 
            if (!(PixelsChecked[x, y]) && CheckPixel((byte*)p, fillcolor)) 
        	{
				p[0]=m_fillcolor; 	 //fill with the color
				PixelsChecked[x,y]=true;
    	...
    	public unsafe void QueueFloodFill4(byte* scan0, int x, int y,Size bmpsize, int stride, byte* fillcolor)
    	{
			//don't go over the edge
        	if ((x <= 0) || (x > bmpsize.Width)) return;
        	if ((y <= 0) || (y > bmpsize.Height)) return;
        ...
        unsafe bool CheckPixel(byte* px, byte* fillcolor) {
            bool ret = true;

            for (byte i = 0; i < 3; i++)
                ret &= (px[i] == fillcolor[i]);
