...
namespace Robocup.MotionControl
{
    ...
    public class RobotInfoTree : BiSearchTree<RobotInfo, Vector2>
    {
        ...
        //maps nodes->parents
        Dictionary<RobotInfo, RobotInfo> parents = new Dictionary<RobotInfo, RobotInfo>();

        int numelements = 0;

        public RobotInfo ClosestStartingAt(RobotInfo point)
        {
            return nnfinder.ClosestGoingFrom(point);
        }

        public RobotInfo ClosestGoingTo(RobotInfo point)
        {
            return nnfinder.ClosestGoingTo(point);
        }

        public void AddNode(RobotInfo node, RobotInfo parent)
        {
            if (parents.ContainsKey(node))
                return;
            numelements++;
            nnfinder.AddInfo(node);
            parents.Add(node, parent);
        }

        public RobotInfo ParentNode(RobotInfo node)
        {
            return parents[node];
        }

        public RobotInfo ClosestStartingAt(Vector2 point)
        {
            return nnfinder.ClosestGoingTo(point);
        }
