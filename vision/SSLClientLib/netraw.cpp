...
bool UDP::addMulticast(const Address &multiaddr,const Address &minterface)
{
  static const bool debug = false;
  struct ip_mreq imreq;
  imreq.imr_multiaddr.s_addr = multiaddr.getInAddr();
  imreq.imr_interface.s_addr = minterface.getInAddr();

  if(debug){
    printf("0x%08X 0x%08X\n",
           (unsigned)minterface.getInAddr(),
           (unsigned)INADDR_ANY);
  }
...
{
  int len = sendto(fd,(char *)data,length,0,&dest.addr,dest.addr_len);
