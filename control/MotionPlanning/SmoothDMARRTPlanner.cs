...
{
    public class SmoothDMARRTPlanner : IPathPlanner
    {
        //Constants-------------------------------------
        ...
        private Tuple<List<Vector2>, double> GetBestPointPath(Team team, int id, IPredictor predictor, RobotInfo desiredState, 
            double avoidBallRadius, RobotPath oldPath, List<Geom> obstacles)
        {
            ...
            { return new Tuple<List<Vector2>,double>(new List<Vector2>(), Double.NegativeInfinity); }
            ...
            return new Tuple<List<Vector2>, double>(bestPath, bestPathScore);
        }
            ...
            RobotPath oldPath, List<Geom> obstacles)
        {
            ...
            if (!Auction.HasWinnerToken(team, id))
            {
                ...
                RobotPath winnerPath = Auction.GetWinnerPath(team, id);
                if (winnerPath != null)
                {
                    foreach (RobotInfo wp in winnerPath.Waypoints)
                        obstacles.Add(new Circle(wp.Position, 3 * Constants.Basic.ROBOT_RADIUS));
                }
                // If we don't get a winner path, we didn't participate in an auction. Nothing to ignore then.
            }
